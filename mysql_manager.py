...
def connect_sql():
    # 连接数据库
    db = pymysql.connect("localhost", "root", "rootroot", "firstDB")
    # 获取游标对象
    cursor = db.cursor()
    # 执行SQL命令
...
def insert_data(user_id='1000', data='Null'):
    if data == 'Null':
        return
    db = pymysql.connect('localhost', 'root', 'rootroot', 'firstDB')
    cursor = db.cursor()
    ...
    if user_id == '1000':
        permissions = 0
    # 插入数据
    sql = "insert into sdk_config(userID, data, permissions) VALUES ('%s', '%s', %d)" % (user_id, json_str, permissions)
    try:
        cursor.execute(sql)
        # 提交到数据库
        db.commit()
